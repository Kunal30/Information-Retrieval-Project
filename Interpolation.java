	...
	public static void main(String args[])throws IOException
{
		...
		HashMap map_interpolation[]=new HashMap[101];
		initializeValues(map_init,map_PF);
		computeInterpolationscore(map_interpolation,map_init,map_PF);
		createRankedList(map_interpolation);
		
}
	public static void createRankedList(HashMap [] map_interpolation)throws IOException
	{
		FileOutputStream fop = null;
		File file;
		file = new File("/home/kunal/Desktop/terrier-3.6/var/results/Retrieval_interpolationresult_ZF1.res");
		...
		{
			ValueComparator bvc = new ValueComparator(map_interpolation[i]);
	        TreeMap<String, Double> sorted_map = new TreeMap<String, Double>(bvc);
	        sorted_map.putAll(map_interpolation[i]);
	        int j=0;
	        for(Entry<String, Double> entry : sorted_map.entrySet()) {
	        	  String key = entry.getKey();
	        	  Double value = entry.getValue();
	        	  String str=(i+" Q0 "+key+" "+j+" "+(value*1000)+" In_expC2"+"\n");
	...
	public static void computeInterpolationscore(HashMap []map_interpolation,HashMap []map_init,HashMap []map_PF)
	{
		double lambda=0.6;
		for(int i=51;i<101;i++)
		{
			map_interpolation[i]=new HashMap();
			Set set = map_init[i].entrySet();
		      ...
		      while(it.hasNext()) {
		         ...
		         {
		        	 double score=(lambda*((double)me.getValue()/score_init_sum[i])+((double)map_PF[i].get(me.getKey())/PF_score_sum[i])*(1-lambda));
		        	 map_interpolation[i].put(me.getKey(),score);
		         }
		         else                                   // for documents exclusively in init
		         {
		        	 double score=((double)me.getValue()/score_init_sum[i])*lambda;
		        	 map_interpolation[i].put(me.getKey(),score);
		         }
		      }
		      ...
		      Iterator it2 = set2.iterator();
		        ...
		        while(it2.hasNext()) {            //for documents exclusively in PF
		          	 ...
		          	 {
		        	 double score=((double)me.getValue()/PF_score_sum[i])*(1-lambda);
		        	 map_interpolation[i].put(me.getKey(),score);
		          	 }
		      }
		}
