	...
	public static void main(String args[])throws IOException
{
		...
		HashMap map_combmnz[]=new HashMap[101];
		initializeValues(map_init,map_PF);
		computeMNZscore(map_combmnz,map_init,map_PF);
		createRankedList(map_combmnz);
		
}
	public static void createRankedList(HashMap [] map_combmnz)throws IOException
	{
		FileOutputStream fop = null;
		File file;
		file = new File("/home/kunal/Desktop/terrier-3.6/var/results/Retrieval_combMNZresult_ZF1.res");
		...
		{
			ValueComparator bvc = new ValueComparator(map_combmnz[i]);
	        TreeMap<String, Double> sorted_map = new TreeMap<String, Double>(bvc);
	        sorted_map.putAll(map_combmnz[i]);
	        int j=0;
	...
	public static void computeMNZscore(HashMap []map_combmnz,HashMap []map_init,HashMap []map_PF)
	{
		for(int i=51;i<101;i++)
		{
			map_combmnz[i]=new HashMap();
			Set set = map_init[i].entrySet();
		      ...
		      while(it.hasNext()) {
		         ...
		         {
		        	 double score=((double)me.getValue()/score_init_sum[i])+((double)map_PF[i].get(me.getKey())/PF_score_sum[i]);
		        	 map_combmnz[i].put(me.getKey(),score);
		         }
		         else                                   // for documents exclusively in init
		         {
		        	 double score=((double)me.getValue()/score_init_sum[i]);
		        	 map_combmnz[i].put(me.getKey(),score);
		         }
		      }
		      ...
		      Iterator it2 = set2.iterator();
		        ...
		        while(it2.hasNext()) {            //for documents exclusively in PF
		          	 ...
		          	 {
		        	 double score=((double)me.getValue()/PF_score_sum[i]);
		        	 map_combmnz[i].put(me.getKey(),score);
		          	 }
		      }
		}
